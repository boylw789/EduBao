...
/**
 * 顾修忠-guxiuzhong@youku.com/gfj19900401@163.com Created by guxiuzhong on
 * 2015/12/29. 上滑悬停控件，底部内容区域支持 ScrollView
 * ，ListView,RecyclerView,GridViewWithHeaderAndFooter(已屏蔽)
 */
public class StickyNavLayout extends LinearLayout {
	...
	public StickyNavLayout(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		stickOffset = a.getDimensionPixelSize(
				R.styleable.StickNavLayout_stickOffset, 0);
		a.recycle();
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mViewPagerMaxHeight = (height >= mViewPagerMaxHeight ? height
				: mViewPagerMaxHeight);
		...
		mTopViewMaxHeight = (topHeight >= mTopViewMaxHeight ? topHeight
				: mTopViewMaxHeight);
		...
		mTop.post(new Runnable() {
			...
			public void run() {
				...
				if (null != mInnerScrollView) {
					Log.d(TAG,
							"mInnerScrollViewHeight:"
									+ mInnerScrollView.getMeasuredHeight());
				}
				if (isStickNav) {
					scrollTo(0, mTopViewHeight);
				}
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		switch (action) {
			...
			case MotionEvent.ACTION_UP:// 处理悬停后立刻抬起的处理
				...
				if (isSticky
						&& /* distance==0.0f */Math.abs(distance) <= mTouchSlop) {
					isSticky = false;
					return true;
				} else {
					isSticky = false;
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (Math.abs(dy) > mTouchSlop) {
					...
					if (mInnerScrollView instanceof ScrollView) {
						...
						if (!isTopHidden
							...
							if (lv.getAdapter() != null
									&& lv.getAdapter().getCount() == 0) {// 当ListView或ScrollView
								// 没有数据为空时
								initVelocityTrackerIfNotExists();
								mVelocityTracker.addMovement(ev);
								mLastY = y;
					...
					// else if (mInnerScrollView instanceof
					// GridViewWithHeaderAndFooter) {
					// GridViewWithHeaderAndFooter gv =
					// (GridViewWithHeaderAndFooter) mInnerScrollView;
					// View c = gv.getChildAt(gv.getFirstVisiblePosition());
					// if (!isTopHidden
					// || (c != null && c.getTop() == 0 && isTopHidden && dy > 0)) {
					// initVelocityTrackerIfNotExists();
					...
					// if (gv.getAdapter() != null && gv.getAdapter().getCount() ==
					// gv.getHeaderViewCount() + gv.getFooterViewCount())
					// {//当没有数据为空时
					// initVelocityTrackerIfNotExists();
					// mVelocityTracker.addMovement(ev);
					// mLastY = y;
	...
	public void scrollTo(int x, int y) {
		...
		if (listener != null) {
			...
			// }
			listener.scrollPercent((float) getScrollY()
					/ (float) mTopViewHeight);
		}
	}
	...
	public interface onStickStateChangeListener {
		 ...
		 *
		 * @param percent
		 *            0~1(向上) or 1~0(向下) 的浮点数
		 */
		void scrollPercent(float percent);
	}

	public void setOnStickStateChangeListener(
			onStickStateChangeListener listener) {
